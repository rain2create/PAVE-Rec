...
本文件属于活跃规划，不是已经确认的研究设计。Phase 1—3 已完成；当前进入
Phase 4 设计确认，其逐项讨论状态记录在 `todo/phase_4_discussion.md`。后续 Phase
...
Phase 3  Real Cheap Path: User Memory + SASRec
   ↓
Phase 4  Deep Segment Evidence + Small Multimodal Reranker Baseline
   ↓
...
Local acceptance evidence：2026-08-03，macOS / Python 3.13，
`168 passed`，package branch coverage `91.62%`，Ruff lint/format passed。

Completion evidence：2026-08-03，candidate commit `af9b16f` 的 project-wide GitHub
Actions run `30796812135` 在 Ubuntu Python 3.10、Ubuntu Python 3.12 和 Windows
Python 3.12 全部通过。Phase 2 Definition of Done 已满足，状态正式记为 `Completed`。
...
本地验收为 `275 passed, 2 skipped`、branch coverage `90.03%`、Ruff clean。completion commit
`5e78957` 的 required GitHub Actions run `30975939269` 已在 Ubuntu/Python 3.10、Ubuntu/Python 3.12
和 Windows/Python 3.12 全部通过。Phase 3 Definition of Done 已满足，状态正式记为 `Completed`。
...
- Top-1-centric score/margin calibration and StopPolicy compatibility
- additive latent/cost artifact contract that preserves Phase 1 trace/replay
...
P4-00—P4-09 与 P4-XG-01 的逐项确认顺序、推荐 baseline 和 implementation
authorization boundary 见 `todo/phase_4_discussion.md`。
...
- 可以分解 recommendation gain、deep encoder/MLLM cost 和 latency
