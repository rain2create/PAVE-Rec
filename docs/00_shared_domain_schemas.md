...
P4-ARCH-02/P4-06 不改变上述公共 shape。每个成功观察的 segment 产生一个 `Evidence`：
`raw_output_ref` 指向 content-addressed selected raw-frame bundle manifest，该 manifest 闭包绑定 2—8 张
canonical RGB frame payloads、mask/timestamps/checksums 和 exact decode/sampling/codec identity。
`embedding_ref` 仅在存在 selector/MLLM/Small-latent 等 exact model-specific derived visual-token manifest 时使用，
否则为 `None`。因为该分支不生成文本或校准概率，`text_summary`、`confidence` 均为 `None`。
Acquisition Need/step 只进入 Evidence event metadata，不进入可跨用户/Query 复用的 frame content artifact。

P4-06 不做 frame/token pooling 或同 item 多 segment 聚合：per-segment Evidence 按 action order 保留在
`ItemEvidenceState.evidence`，`aggregated_attributes` 只保存紧凑 inventory，P4 baseline 的
`evidence_embedding_ref` 保持 `None`；P4-07 native-frame MLLM Reranker 从各 `Evidence.raw_output_ref` 加载
原始帧并输出候选 scores。只有 manifest/payload 原子发布并完成 checksum/schema/identity 校验后才允许
`succeeded`。任何 decode、artifact、model 或 cache failure 都必须为 `failed`、不携带 Evidence、也不触发
score update。Model-specific token cache 必须绑定 source frame checksum 和 exact checkpoint/processor；个性化
MLLM scores 不得跨用户/状态复用。
